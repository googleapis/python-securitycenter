...
from google.cloud.securitycenter_v1p1beta1.proto.notification_config_pb2 import NotificationConfig
import pytest
...
def test_create_notification_config():
  created_notification_config = snippets_notification_configs.create_notification_config(ORG_ID, CONFIG_ID, PUBSUB_TOPIC)
  assert created_notification_config is not None

  cleanup_notification_config(CONFIG_ID)

def test_delete_notification_config(deleted_notification_config):
  assert snippets_notification_configs.delete_notification_config(ORG_ID, CONFIG_ID) is True
  
def test_get_notification_config(new_notification_config):
  retrieved_config = snippets_notification_configs.get_notification_config(ORG_ID, CONFIG_ID)
...
def test_update_notification_config(new_notification_config):
  updated_config = snippets_notification_configs.update_notification_config(ORG_ID, CONFIG_ID, PUBSUB_TOPIC)
  assert updated_config is not None

  cleanup_notification_config(CONFIG_ID)

def test_receive_notifications():
  assert snippets_notification_receiver.receive_notifications(PROJECT_ID, PUBSUB_SUBSCRIPTION) is True